...
class PDFGeneratorView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        with open(os.path.join(settings.STATIC_ROOT, "images/Logo_Vertical_IDEXUD_Final-1-3-1.png"), 'rb') as image_file:
            logoIdexud = base64.b64encode(image_file.read()).decode('utf-8')
        with open(os.path.join(settings.STATIC_ROOT, "images/EscudoUD.png"), 'rb') as image_file:
            logoUd = base64.b64encode(image_file.read()).decode('utf-8')
        with open(os.path.join(settings.STATIC_ROOT, "images/Firma.png"), 'rb') as image_file:
